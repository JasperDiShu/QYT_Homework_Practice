...
def syslog_show_log_source(host, user, password, database):
    ...
    cursor = conn.cursor()
    # 提取时间与CPU利用率信息
    cursor.execute("select log_source as log,COUNT(*) as count from syslogdb group by log_source")
...
if __name__ == '__main__':
    syslog_show_log_source('192.168.200.136', 'shudidbuser', 'shudidbpassword', 'shudidb')
